...
inline const glm::vec2& position(const DFSPH::Particle& p) { return p.position; }
inline const glm::vec2& position(const DFSPH::GhostParticle& p) { return p.position; }
inline float pressure(const DFSPH::Particle& p) { return 0.0; }
inline const glm::vec2& velocity(const DFSPH::Particle& p) { return p.velocity[0]; }

int main(int argc, char** argv)
{
    Viewer viewer;
    viewer.mWindow.title = "Divergence-Free SPH";
    ...
    viewer.onGui([&](Window& window, double dt)
    {
        ...
        ImGui::Begin("Settings", nullptr, ImGuiWindowFlags_NoResize | ImGuiWindowFlags_NoSavedSettings | ImGuiWindowFlags_NoCollapse);
        {
            ...
            ImGui::TextColored(ImVec4(0.9, 0.6, 0.6, 1.0), "Num. Boundary Particles: %llu", simulation.boundaryParticles.size());
            {
                ...
                ImGui::TextColored(ImVec4(1.0, 0.4, 0.4, 1.0), "Total : %4.3f ms", perf_monitor.time("dfsph_total"));
                ImGui::TextColored(ImVec4(1.0, 0.5, 0.0, 1.0), "\tforce: %4.3f ms",  perf_monitor.time("dfsph_force"));
                ImGui::TextColored(ImVec4(1.0, 0.5, 0.0, 1.0), "\tpred. vel: %4.3f ms", perf_monitor.time("dfsph_pred_vel"));
                ImGui::TextColored(ImVec4(0.5, 0.5, 1.0, 1.0), "\tdensity solver: %4.3f ms", perf_monitor.time("dfsph_density_solve"));
                ImGui::TextColored(ImVec4(1.0, 0.5, 0.0, 1.0), "\tupdate pos: %4.3f ms", perf_monitor.time("dfsph_update_pos"));
                ImGui::TextColored(ImVec4(0.1, 0.8, 0.0, 1.0), "\tnnfill: %4.3f ms", perf_monitor.time("dfsph_nnfill"));
                ImGui::TextColored(ImVec4(0.5, 0.5, 1.0, 1.0), "\tdensity / factor: %4.3f ms", perf_monitor.time("dfsph_density_alpha"));
                ImGui::TextColored(ImVec4(0.5, 0.5, 1.0, 1.0), "\tdivergence solver: %4.3f ms", perf_monitor.time("dfsph_divergence_solve"));
                ImGui::TextColored(ImVec4(1.0, 0.5, 0.0, 1.0), "\tupdate vel: %4.3f ms", perf_monitor.time("dfsph_update_vel"));
            }
            ...
            {
                ImGui::SliderInt("density solver iter", &simulation.densitySolverIter, 0, 32);
                ImGui::SliderInt("divergence solver iter", &simulation.divergenceSolverIter, 0, 32);
    ...
    viewer.onKey([&] (Window& window, Keyboard& keyboard, int key, int mod, bool press)
    {
        ...
        if(key == GLFW_KEY_SPACE)
        {
            ...
            /* i just recompute alpha and neighbourhood for the whole scene again */
            simulation.stepFillNNsearch();
            simulation.stepDensityApha();
        }
