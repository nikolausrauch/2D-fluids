...
inline const glm::vec2& position(const IISPH::Particle& p) { return p.position; }
inline const glm::vec2& position(const IISPH::GhostParticle& p) { return p.position; }
inline float pressure(const IISPH::Particle& p) { return p.pressure; }
inline const glm::vec2& velocity(const IISPH::Particle& p) { return p.velocity[0]; }
    ...
    viewer.onGui([&](Window& window, double dt)
    {
        ...
        ImGui::Begin("Settings", nullptr, ImGuiWindowFlags_NoResize | ImGuiWindowFlags_NoSavedSettings | ImGuiWindowFlags_NoCollapse);
        {
            ...
            ImGui::TextColored(ImVec4(0.9, 0.6, 0.6, 1.0), "Num. Boundary Particles: %llu", simulation.boundaryParticles.size());
            {
                ...
                ImGui::TextColored(ImVec4(1.0, 0.4, 0.4, 1.0), "Total : %4.3f ms", perf_monitor.time("iisph_total"));
                ImGui::TextColored(ImVec4(0.1, 0.8, 0.0, 1.0), "\tnnfill: %4.3f ms",  perf_monitor.time("iisph_nnfill"));
                ImGui::TextColored(ImVec4(1.0, 0.5, 0.0, 1.0), "\tpredict velocity: %4.3f ms", perf_monitor.time("iisph_predvel"));
                ImGui::TextColored(ImVec4(1.0, 0.5, 0.0, 1.0), "\tdensity advection: %4.3f ms", perf_monitor.time("iisph_advectdensity"));
                ImGui::TextColored(ImVec4(0.5, 0.5, 1.0, 1.0), "\tpressure solver: %4.3f ms", perf_monitor.time("iisph_pressuresolve"));
                ImGui::TextColored(ImVec4(0.5, 0.5, 1.0, 1.0), "\t\tcompute coeff: %4.3f ms", perf_monitor.time("iisph_coeff"));
                ImGui::TextColored(ImVec4(0.5, 0.5, 1.0, 1.0), "\t\tupdate pressure: %4.3f ms", perf_monitor.time("iisph_updatepress"));
                ImGui::TextColored(ImVec4(0.5, 0.5, 1.0, 1.0), "\t\tpressure force: %4.3f ms", perf_monitor.time("iisph_pressureforce"));
                ImGui::TextColored(ImVec4(0.5, 0.5, 1.0, 1.0), "\tintegrate: %4.3f ms", perf_monitor.time("iisph_integrate"));
            }
