...
inline const glm::vec2& position(const PCISPH::Particle& p) { return p.position[0]; }
inline const glm::vec2& position(const PCISPH::GhostParticle& p) { return p.position; }
inline float pressure(const PCISPH::Particle& p) { return p.pressure; }
inline const glm::vec2& velocity(const PCISPH::Particle& p) { return p.velocity[0]; }

int main(int argc, char** argv)
{
    Viewer viewer;
    viewer.mWindow.title = "Predictive-Corrective Incompressible SPH";
    ...
    viewer.onGui([&](Window& window, double dt)
    {
        ...
        ImGui::Begin("Settings", nullptr, ImGuiWindowFlags_NoResize | ImGuiWindowFlags_NoSavedSettings | ImGuiWindowFlags_NoCollapse);
        {
            ...
            ImGui::TextColored(ImVec4(0.9, 0.6, 0.6, 1.0), "Num. Boundary Particles: %llu", simulation.boundaryParticles.size());
            {
                ...
                ImGui::TextColored(ImVec4(1.0, 0.4, 0.4, 1.0), "Total : %4.3f ms", perf_monitor.time("pcisph_total"));
                ImGui::TextColored(ImVec4(0.1, 0.8, 0.0, 1.0), "\tnnfill: %4.3f ms",  perf_monitor.time("pcisph_nnfill"));
                ImGui::TextColored(ImVec4(1.0, 0.5, 0.0, 1.0), "\text force: %4.3f ms", perf_monitor.time("pcisph_nopressforce"));
                ImGui::TextColored(ImVec4(0.5, 0.5, 1.0, 1.0), "\tprediction correction: %4.3f ms", perf_monitor.time("pcisph_predcorr"));
                ImGui::TextColored(ImVec4(0.5, 0.5, 1.0, 1.0), "\t\tprediction: %4.3f ms", perf_monitor.time("pcisph_predict"));
                ImGui::TextColored(ImVec4(0.5, 0.5, 1.0, 1.0), "\t\tpressure correction: %4.3f ms", perf_monitor.time("pcisph_presscorr"));
                ImGui::TextColored(ImVec4(0.5, 0.5, 1.0, 1.0), "\t\tpressure force: %4.3f ms", perf_monitor.time("pcisph_pressforce"));
                ImGui::TextColored(ImVec4(0.5, 0.5, 1.0, 1.0), "\tintegrate: %4.3f ms", perf_monitor.time("pcisph_integrate"));
            }
            ...
            ImGui::TextColored(ImVec4(0.6, 0.8, 0.6, 1.0), "Numeric: ");
            {
                ...
                ImGui::SliderInt("min solver steps", &simulation.numIterations, 0, 100);
                ImGui::SliderFloat("error threshold", &simulation.errorThresh, 0.001, 0.2, "%.3f");
            }
