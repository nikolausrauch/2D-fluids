...
void NearestNeighbor::fillGrid(std::vector<Particle> &particles, float radius)
{
    for(unsigned int i = 0; i < particles.size(); i++)
    {
        ...
        glm::ivec2 bucket = glm::floor(p.position[1] / (1.0f*radius));
    ...
    for(int x = -1; x <= 1; x++)
    {
        for(int y = -1; y <= 1; y++)
        {
            ...
            for(unsigned int j : mHashgrid[ hash % mHashgrid.size() ])
            {
                ...
                if(glm::length(p.position[1] - pj.position[1]) < radius )
                {
...
void NearestNeighbor::clear(unsigned int p_index)
{
    mHashgrid[p_index].clear();
}
